...
def ats_scoring_non_tech_v2(file_path, applicant_name="Candidate"):
    ...
    # Check the first line for applicant name (if it's not explicitly provided)
    ...
    # LinkedIn Detection - Regex to detect LinkedIn profile links (both old and new formats)
    linkedin_detection = "YES" if re.search(r"linkedin\.com/(in|pub)/[A-Za-z0-9\-_%]+", text_lower) else "NO"

    # Portfolio Link Detection - Detects URLs related to personal portfolios, GitHub, etc.
    portfolio_detection = "YES" if re.search(r"https?://[\w.-]+\.(?:me|dev|design|studio|site|xyz|io|app|tech|works|space|online|digital|portfolio|com|net|org)(?:/\S*)?", text_lower) else "NO"
    ...
    criteria = [
        ...
        ("LinkedIn Profile", 20, "Link present (2), Headline & Summary (3), Skills & Endorsements (4), Portfolio/Cert links (4), Activity/posts (7)"),
    ...
    # LinkedIn scoring (presence of sections: Headline, Summary, Experience, etc.)
    linkedin_score = 0
    linkedin_subcriteria = []
    if linkedin_detection == "YES":
        linkedin_score = 2  # Link presence
        linkedin_subcriteria.append({"name": "Link present", "score": 2, "insight": "LinkedIn link detected."})

        # Check for common LinkedIn sections
        sections_found = [
            "headline", "about", "experience", "projects", "education", "skills", "certifications"
        ]
        section_score = 0
        for section in sections_found:
            if re.search(rf"\b{section}\b", text_lower):
                section_score += 2  # Assign a score per section found
        linkedin_score = min(section_score, 20)  # Max LinkedIn score is 20
        linkedin_subcriteria.append({"name": "Sections detected", "score": section_score, "insight": f"{section_score} LinkedIn sections found."})

    else:
        linkedin_score = 0
        linkedin_subcriteria.append({"name": "Link present", "score": 0, "insight": "No LinkedIn link found."})

    # Portfolio scoring (presence of portfolio links)
    portfolio_score = 0
    portfolio_subcriteria = []
    if portfolio_detection == "YES":
        portfolio_score = 20
        portfolio_subcriteria.append({"name": "Portfolio link present", "score": portfolio_score, "insight": "Portfolio link detected."})
    else:
        portfolio_score = 0
        portfolio_subcriteria.append({"name": "Portfolio link present", "score": 0, "insight": "No portfolio link found."})

    # Simple evaluation logic (expandable for other criteria)
    for name, weight, explanation in criteria:
        score = 0
        recs = []
        ...
        elif name == "LinkedIn Profile":
            score = linkedin_score
            if linkedin_score == 0:
                recs.append("Add a LinkedIn link to your resume and ensure key sections are present.")
    ...
    return {
        ...
        "contact_detection": contact_detection,
        "linkedin_detection": linkedin_detection,
        "portfolio_detection": portfolio_detection,
        "ats_score": score_breakdown["Keyword Integration"]["score"],
        "overall_score_average": overall_score_average,
        "score_breakdown": score_breakdown,
        "pie_chart_image": pie_chart_image,
